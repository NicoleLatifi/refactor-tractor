...
class Activity { 
  constructor(data, userRepository) { // instantiated with data from dataset, and userRepository to access a specific user?
    ...
    this.numSteps = data.numSteps; // stores the number of steps taken (per day?) ???
  ...
  compareStepGoal(userRepository) { // not sure this is being invoked anywhere?
    ...
    this.reachedStepGoal = this.numSteps >= userStepGoal; // returns a boolean based on whether goal was met or not, stores in this.reachedStepGoal
  }
}
